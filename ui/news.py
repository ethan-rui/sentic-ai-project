    ...
    for topic in TOPICS:
        ...
        for x in range(sets):
            ...
            while True:
                try:
                    links.append(
                        soup.select(f"#Fin-Stream > ul > li:nth-child({x}) a")[0][
                            "href"
                        ]
                    )
...
def new_sentiment():
    ...
    if logs == None:
        ...
        data = dict(
            zip(
                ["Title", "Sentiment"],
                [list(ticker_data["Title"]), list(ticker_data["score"])],
            )
        )
        return {"news": data}
    else:
        ...
        data = dict(
            zip(
                ...
                [
                    list(ticker_data["Title"]),
                    list(ticker_data["score"]),
                    list(ticker_data["link"]),
                ],
            )
        )
        return {"news": data}
...
def news_analysis(SETS: int = 5, THREADS: int = 1):
    ...
    # print(tickers)
    data = scrape_news(
        name="crypto&stock", sets=SETS, crypto=True, stock=True, save=True
    )  # 5 sets scrapes abt "18 hours worth" of news

    data = cleaner.label_tickers(
        df=data,
        tickers=tickers,
        targeted_col_name="News",
        new_col_name="Ticker",
        dfname=NAME,
        save=True,
    )
